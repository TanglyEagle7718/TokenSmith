...
"""
video_indexer.py
Processes videos using llama-cpp-python and OpenAI Whisper with stability fixes.
"""
...
# from src.index_builder import preprocess_for_bm25 # Removed to break circular dependency
...
class VideoIndexer:
    def __init__(
        ...
        # Load Whisper model (suppress FP16 warning on CPU)
        ...
        # Load GGUF Model with stability settings
        # The instructions say to use this specific model
    ...
    def extract_transcript(self, video_path: str) -> List[Dict]:
        """Transcribes the video audio using OpenAI Whisper and returns timestamped segments."""
        print(f"Transcribing {video_path}...")
        start_time = time.time()
        # Ensure we use CPU explicitly if needed, though whisper handles it
    ...
    def extract_frames(self, video_path: str, interval_sec: int = 5) -> List[np.ndarray]:
        """Extracts frames from the video using fast-seeking."""
        start_time = time.time()
        images = []
    ...
    def embed_text(self, texts: List[str]) -> np.ndarray:
        """Generates embeddings for text chunks using llama-cpp with mean pooling."""
        ...
        for text in tqdm(texts, desc="Embedding Chunks", leave=False):
            try:
                # Qwen3-VL-Embedding returns token-level embeddings
                response = self.model.create_embedding(text)
                emb_list = response['data'][0]['embedding']
                
                # Convert to numpy and mean-pool
                emb_array = np.array(emb_list, dtype=np.float32)
                if emb_array.ndim == 2: # [tokens, dim]
    ...
    def embed_frames(self, frames: List[np.ndarray]) -> np.ndarray:
        ...
        for frame in tqdm(frames, desc="Embedding Frames", leave=False):
            try:
                ...
                # Use create_chat_completion to process multimodal input
                messages = [
                    {
                        "role": "user",
                        "content": [
                ...
                # Extract pooled embedding (last token)
    ...
    def process_videos(self, video_dir: str):
        """Processes only new videos by checking existing metadata."""
        ...
        for v_file in new_files:
            ...
            # 0. Add Title Chunk for better keyword matching
            ...
            try:
                ...
                if segments:
                    # Group segments into small groups to avoid too many tiny chunks
                    # but keep enough granularity for timestamps.
                    group_size = 3
                ...
                print(f"Error processing transcript for {v_file.name}: {e}")
            ...
            try:
                ...
                print(f"Error processing frames for {v_file.name}: {e}")
